  ...
  function setPreview(i, slot, src) {
    ...
    if (img) {
      ...
      img.onload  = () => { if (specs) ImageAnalysis.render(specs, img.src, { minW: IND_MIN_W, minH: IND_MIN_H, minRatio: IND_MIN_RATIO, enforced: true }); };
  ...
  /* ─── quitar imagen (deja el slot vacío, no borra el archivo del servidor —
     la limpieza de archivos huérfanos se hace desde la página "Imágenes") ── */
  function clearImage(i, slot) {
    industrias[i][`imagen${slot}`] = '';
