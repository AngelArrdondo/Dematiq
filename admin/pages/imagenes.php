...
<script>
  ...
  async function loadProfile(){
    try{ const res=await fetch('../api/profile.php?action=get'); const json=await res.json(); if(!json.ok)return; profileData=json.data;
      ['profilePrimerNombre','profileSegundoNombre','profileApellidoPaterno','profileApellidoMaterno','profileEmail','profileTelefono','profileUsername'].forEach((id,i)=>{
        const keys=['primer_nombre','segundo_nombre','apellido_paterno','apellido_materno','email_contacto','telefono','username'];
        document.getElementById(id).value=json.data[keys[i]]||'';
      });
  ...
  async function saveProfileInfo(){
    ...
    if(!pn||!ap){showToast('Nombre y apellido son requeridos','error');return;}
    const btn=document.getElementById('infoSaveBtn'); btn.disabled=true; btn.textContent='Guardando…';
    const fd=new FormData(); ['action','primer_nombre','segundo_nombre','apellido_paterno','apellido_materno','email_contacto','telefono'].forEach((k,i)=>{
      const ids=[null,'profilePrimerNombre','profileSegundoNombre','profileApellidoPaterno','profileApellidoMaterno','profileEmail','profileTelefono'];
      if(i===0) fd.append(k,'update_info'); else fd.append(k,document.getElementById(ids[i]).value.trim());
    });
    ...
    }catch{showToast('Error','error');}finally{btn.disabled=false;btn.textContent='Guardar cambios';}
  }
  async function uploadAvatar(input){
    if(!input.files[0]||input.files[0].size>2*1024*1024){if(input.files[0])showToast('Máximo 2 MB','error');input.value='';return;}
    ...
    }catch{showToast('Error','error');}finally{eb.style.opacity='1';input.value='';}
  }
  async function changePassword(){
    ...
    if(!cur||!np||!cp||np!==cp||np.length<8){showToast(np!==cp?'No coinciden':'Min 8 caracteres','error');return;}
    ...
    }catch{showToast('Error','error');}finally{btn.disabled=false;btn.textContent='Cambiar contraseña';}
  }
