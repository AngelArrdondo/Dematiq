...
<script>
  ...
  async function loadProfile() {
    try {
      const res = await fetch('../api/profile.php?action=get'); const json = await res.json();
      if (!json.ok) return; profileData = json.data;
  ...
  async function saveProfileInfo() {
    const pn = document.getElementById('profilePrimerNombre').value.trim();
    const ap = document.getElementById('profileApellidoPaterno').value.trim();
    if (!pn) { showToast('El primer nombre es requerido','error'); return; }
    if (!ap) { showToast('El apellido paterno es requerido','error'); return; }
    const btn = document.getElementById('infoSaveBtn'); btn.disabled=true; btn.textContent='Guardando…';
    const fd = new FormData();
    fd.append('action','update_info'); fd.append('primer_nombre',pn); fd.append('segundo_nombre',document.getElementById('profileSegundoNombre').value.trim());
  ...
  async function changePassword() {
    ...
    if(!cur||!np||!cp){showToast('Todos los campos son requeridos','error');return;}
    if(np!==cp){showToast('Las contraseñas no coinciden','error');return;}
    if(np.length<8){showToast('Mínimo 8 caracteres','error');return;}
