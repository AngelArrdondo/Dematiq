    ...
    <div class="admin-card">
      <div class="admin-card-header">
        <div class="admin-card-title">
          ...
          <span class="slides-count-badge" id="servicios-count"></span>
        </div>
...
<script>
  ...
  async function uploadImage(i, input) {
    ...
    try {
      ...
      const json = await res.json();
      if (json.ok) {
        servicios[i].image = json.path;
        const pi = document.getElementById('imgPath' + i);
        if (pi) pi.value = json.path;
        showToast('Imagen subida correctamente');
      } else {
        showToast(json.error || 'Error al subir', 'error');
        setPreview(i, servicios[i].image ? '../../' + servicios[i].image : '');
      }
    } catch { showToast('Error de conexión', 'error'); setPreview(i, servicios[i].image ? '../../' + servicios[i].image : ''); }
  ...
  function renderServicios() {
    ...
    const badge = document.getElementById('servicios-count');
    if (badge) badge.textContent = servicios.length;
    c.innerHTML = '';
    if (servicios.length === 0) {
      c.innerHTML = '<p style="text-align:center;padding:28px;color:var(--text-lt);font-size:.88rem">No hay servicios. Haz clic en <strong>Agregar</strong> para añadir uno.</p>';
      return;
    }
      ...
      div.innerHTML = `
        <div class="slide-card-header">
          <div class="slide-badge">
            ...
            <span class="slide-header-title${s.nombre ? '' : ' empty'}" id="itemTitle${i}">${esc(s.nombre) || 'Nuevo servicio'}</span>
          </div>
          <button class="btn-rm" onclick="removeServicio(${i})" title="Eliminar" style="flex-shrink:0">${trashIcon}</button>
        </div>
        <div class="slide-body">
          <div class="slide-img-col">
            ...
            <input type="text" class="slide-path-input" id="imgPath${i}" value="${esc(s.image)}"
              oninput="servicios[${i}].image=this.value;setPreview(${i},this.value?'../../'+this.value:'')"
              placeholder="assets/images/general/img1.webp">
          </div>
